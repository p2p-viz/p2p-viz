...
function gossipNetworkManager(context) {
  ...
  manager.on_data = (data, peer) => {
    ...
    if (manager.__forwarded_hashes.length >= manager.MAX_HASHES_BUFFER) {
      manager.__forwarded_hashes.splice(
        0,
        Math.ceil(manager.MAX_HASHES_BUFFER * 0.1)
      );
    }
    ...
    dispatchContextEvent(
      context,
      "log",
      `Data recieved from immediate ${peer}`
    );
    if (data.type && data.type == "general") {
      ...
      dispatchContextEvent(
        context,
        "log",
        `Connection to ${data.payload.id} ready`
      );
    }
    ...
    if (data.target && data.target == context.__peer_id) {
      ...
      data.target &&
      context.__network_manager.__conntected_peers.indexOf(data.target) != -1
    ) {
  ...
  manager.prepareConnection = async (conn, type) => {
    if (context.__network_manager.__connected_peers.indexOf(conn.peer) == -1) {
      ...
      if (
        manager.__connected_peers.length == manager.MAX_PEERS &&
        context.__network_manager.__is_registered
      ) {
      ...
      conn.on("close", async (data) => {
        const index = context.__network_manager.__connected_peers.indexOf(
          conn.peer
        );
        if (index != -1) {
        ...
        dispatchContextEvent(
          context,
          "log",
          `Connection to ${conn.peer} closed`
        );
      });
      ...
      if (context.__dynamic_network) {
        setTimeout(
          ...
          context.__dynamic_network_timeout
            ? context.__dynamic_network_timeout
            : 20000
        );
      }
      ...
      dispatchContextEvent(
        context,
        "log",
        `Connected to ${conn.peer} with conneciton type ${type}`
      );
    } else {
      conn.close();
    }
  ...
  manager.tryConnectToPeers = async (peerList, numTries) => {
    if (
      manager.__connected_peers.length >= manager.MAX_PEERS * 0.5 ||
      !context.__connected
    ) {
      return;
    }
  ...
  manager.sendMessage = (message, messageType, targetPeer) => {
    ...
    if (
      targetPeer &&
      context.__network_manager.__connected_peers.indexOf(targetPeer) != -1
    ) {
...
function meshNetworkManager(context) {
  ...
  manager.sendMessage = (message, message_type, target_peer) => {
    ...
    if (
      target_peer &&
      context.__network_manager.__connected_peers.indexOf(target_peer) != -1
    ) {
...
function connectToIntermediateServer(context) {
  ...
  context.__peer.on("close", () => {
    ...
    dispatchContextEvent(
      context,
      "log",
      `Disconnected from the network ice server`
    );
...
export default function p2pnet() {
  ...
  context.__create_db = (dbName) => {
    ...
    dbObj.__update = (data) => {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (!(key in context.__dbs[dbName].data) && value.value != null) {
          ...
          if (
            new Date(current_value.last_updated) < new Date(value.last_updated)
          ) {
            if (value.value == null) {
    ...
    context.on("node_connect", (node_id) => {
      ...
      dispatchContextEvent(
        context,
        "log",
        `Share database ${dbName} with ${node_id}`
      );
      context.__network_manager.sendMessage(
        {
          type: "update_db",
  ...
  context.on("error", (message) => {
    console.error("p2pnet error: " + message);
  });
